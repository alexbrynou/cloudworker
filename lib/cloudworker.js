...
class Cloudworker {
  constructor (workerScript, {debug = false, bindings = {}, enableCache = false} = {}) {
    ...
    this.httpRoutes = {
      '/svc_edge_worker/purge_cache' : this.purgeCache.bind(this)
    }
  }
  ...
  async _handle (req, res) {
    ...
    const urlParts = urlInstance.parse(url, true),
        urlPathname = urlParts.pathname;

    if (this.httpRoutes[urlPathname]) {
      return await this.httpRoutes[urlPathname](req, res);
    }
  ...
  async purgeCache(req, res) {
    const queryObject = urlInstance.parse(req.url, true).query;

    if (queryObject.tags) {
      const tagsString = queryObject.tags.split(',');
      console.log(`PURGED for ${tagsString}`);
      this.context.caches.default.resetByTag(tagsString);
    }
